...
function one_lineup_Type_4(hitters, pitchers, lineups, num_overlap, num_hitters, num_pitchers, catcher, first_baseman, second_baseman, third_baseman, shortstop, outfielders, num_teams, hitters_teams, pitchers_teams, pitchers_opponents)
...
function create_lineups(num_lineups, num_overlap, path_hitters, path_pitchers, formulation, path_to_output)
    ...
    # Populate player_info with the corresponding information
    for j=1:size(teams)[1]
        if pitchers[1, :Team] == teams[j]
            player_info[j] =1
        end
    end
    pitchers_teams = player_info'


    for i=2:num_pitchers
        player_info = zeros(Int, size(teams)[1])
        for j=1:size(teams)[1]
            if pitchers[i, :Team] == teams[j]
                player_info[j] =1
            end
        end
        pitchers_teams = vcat(pitchers_teams, player_info')
    end
    ...
    the_lineup= formulation(hitters, pitchers, hcat(zeros(Int, num_hitters + num_pitchers), zeros(Int, num_hitters + num_pitchers)), num_overlap, num_hitters, num_pitchers, catcher, first_baseman, second_baseman, third_baseman, shortstop, outfielders, num_teams, hitters_teams, pitchers_teams, pitchers_opponents)
    the_lineup2 = formulation(hitters, pitchers, hcat(the_lineup, zeros(Int, num_hitters + num_pitchers)), num_overlap, num_hitters, num_pitchers, catcher, first_baseman, second_baseman, third_baseman, shortstop, outfielders, num_teams, hitters_teams, pitchers_teams, pitchers_opponents)
    tracer = hcat(the_lineup, the_lineup2)
    for i=1:(num_lineups-2)
        try
            thelineup=formulation(hitters, pitchers, tracer, num_overlap, num_hitters, num_pitchers, catcher, first_baseman, second_baseman, third_baseman, shortstop, outfielders, num_teams, hitters_teams, pitchers_teams, pitchers_opponents)
