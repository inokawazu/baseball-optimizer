...
num_lineups = 100
num_overlap = 4
path_hitters = "Desktop/DFS 2018/DraftKings_hitters.csv"
path_pitchers = "Desktop/DFS 2018/DraftKings_pitchers.csv"
path_to_output = "output.csv"

# This is a function that creates one lineup using the Type 4 formulation from the paper
function one_lineup_Type_4(hitters, pitchers, lineups, num_overlap, num_hitters, num_pitchers, pitcher, catcher, first_baseman, second_baseman, third_baseman, shortstop, outfielders, num_teams, hitters_teams, pitchers_opponents)
    ...
    @constraint(m, sum{pitchers_lineup[i], i=1:num_pitchers} == 2)
    
    # Eight Hitters constraint
    @constraint(m, sum{hitters_lineup[i], i=1:num_hitters} == 8)
    
    # 2 pitchers constraint
    @constraint(m, sum{pitcher[i]*pitchers_lineup[i], i=1:num_pitchers} == 2)
    
    # 1 catcher constraint
    @constraint(m, sum{(catcher)[i]*hitters_lineup[i], i=1:num_hitters} == 1)
    
    # 1 first baseman constraint
    @constraint(m, sum{(first_baseman)[i]*hitters_lineup[i], i=1:num_hitters} == 1)
    
    # 1 second basemen constraint
    @constraint(m, sum{second_baseman[i]*hitters_lineup[i], i=1:num_hitters} == 1)
    
    
    # 1 third basemen constraint
    @constraint(m, sum{third_baseman[i]*hitters_lineup[i], i=1:num_hitters} == 1)
    
    
    # 1 shortstop constraint
    @constraint(m, sum{shortstop[i]*hitters_lineup[i], i=1:num_hitters} == 1)
    
    
    # 3 outfielders constraint
    @constraint(m, sum{outfielders[i]*hitters_lineup[i], i=1:num_hitters} == 3)
    

    # Financial Constraint
    @constraint(m, sum{hitters[i,:Salary]*hitters_lineup[i], i=1:num_hitters} + sum{pitchers[i,:Salary]*pitchers_lineup[i], i=1:num_pitchers} <= 50000)
    ...
    constr = @constraint(m, [i=1:num_teams], sum{hitters_teams[t, i]*hitters_lineup[t], t=1:num_hitters} <= 5*used_team[i])
    @constraint(m, sum{used_team[i], i=1:num_teams} == 2)


    # No pitchers going against hitters
    constr = @constraint(m, [i=1:num_pitchers], 5*pitchers_lineup[i] + sum{pitchers_opponents[k, i]*hitters_lineup[k], k=1:num_hitters}<=5)
...
function create_lineups(num_lineups, num_overlap, path_hitters, path_pitchers, formulation, path_to_output)
    ...
    # pitchers stores the information on which players are pitchers
    pitcher = Array{Int64}(0)
    ...
    for i =1:num_hitters
        ...
        elseif contains(pos,"1B")
            ...
            first_baseman=vcat(first_baseman,fill(1,1))
            second_baseman=vcat(second_baseman,fill(0,1))
            third_baseman=vcat(third_baseman,fill(0,1))
            shortstop=vcat(shortstop,fill(0,1))
            outfielders=vcat(outfielders,fill(0,1))
    ...
    for i =1:num_pitchers
        pos = pitchers[i,:Position];
        if contains(pos,"P")
            pitcher=vcat(pitcher,fill(1,1))
        end
    end
    ...
    the_lineup= formulation(hitters, pitchers, hcat(zeros(Int, num_hitters + num_pitchers), zeros(Int, num_hitters + num_pitchers)), num_overlap, num_hitters, num_pitchers, pitcher, catcher, first_baseman, second_baseman, third_baseman, shortstop, outfielders, num_teams, hitters_teams, pitchers_opponents)
    the_lineup2 = formulation(hitters, pitchers, hcat(the_lineup, zeros(Int, num_hitters + num_pitchers)), num_overlap, num_hitters, num_pitchers, pitcher, catcher, first_baseman, second_baseman, third_baseman, shortstop, outfielders, num_teams, hitters_teams, pitchers_opponents)
    tracer = hcat(the_lineup, the_lineup2)
    for i=1:(num_lineups-2)
        try
            thelineup=formulation(hitters, pitchers, tracer, num_overlap, num_hitters, num_pitchers, pitcher, catcher, first_baseman, second_baseman, third_baseman, shortstop, outfielders, num_teams, hitters_teams, pitchers_opponents)
